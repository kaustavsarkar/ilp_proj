...
<p><strong>View & Update Items</strong></p>
</header>
...
<div align="center">
<form name="deleteall" >
<% ArrayList<Items> itemsList=(ArrayList<Items>) request.getAttribute("itemsList"); 
if(itemsList.size()> 0){
	
%>
<table border="1"  border-collapse="collapse" cellspacing="0">
<tr><td>SELECT</td><td>ITEM ID</td><td>ITEM NAME</td><td>DEPARTMENT ID</td><td>PRICE</td><td>QUANTITY</td><td colspan="2">Operations</td></tr>
<%
for(Items item : itemsList)
{
%>
<tr>
	<td><input type="checkbox" name="selectid" value="<%=item.getItemId()%>" /></td>
	...
	<%
	//int b=2;%> 
	<td><a href="<%=request.getContextPath()%>/AdminController?action=searchitems&itemid=<%=item.getItemId()%>&value=view">Update</a>
	<td><a href="<%=request.getContextPath()%>/AdminController?action=searchitems&itemid=<%=item.getItemId()%>&value=delete">Delete</a>
</tr>
<%
}
%>
...
<br></br>
<%--
<a href="<%=request.getContextPath()%>/AdminController?action=searchitems&value=deleteSelected">
 --%>
<input type="submit" value="Delete All" action="<%=request.getContextPath()%>/AdminController" />

<input type="hidden" name="action" value="deleteallitems"><br>
<br></br>
<a href="<%=request.getContextPath() %>/JSP/manageitems.jsp" >Manage Items</a><br></br>
