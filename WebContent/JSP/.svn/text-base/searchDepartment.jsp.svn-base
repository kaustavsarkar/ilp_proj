<%@page import="com.tcs.ilp.POJO.Department"%>
...
<div >
<h1 align="center"><u>View Department</u></h1>
</div>
<body>
	<%
	...
	<form onsubmit="return validateUpdateDepartment()" name="udpatedepartmentForm" method="post" action="<%=request.getContextPath()%>/AdminController">
		<table>
			
			<tr>
					<td>Department ID:</td>
					<td><input type="text" name="departmentid" id="departmentid" value="<%=department.getDepartmentId()%>" readonly="readonly"></td>
			
			</tr>
			
			<tr>
				<td>Department Name:</td>
				<td><input type="text" name="departmentname" id="departmentname" value="<%=department.getDepartmentName()%>"></input></td>
		</table>
		<input type="submit" value="updatedepartment" name="action" onclick="<%=request.getContextPath()%>/AdminController?departmentid=<%=department.getDepartmentId()%>">
		<%-- <input type="submit" value="deletedepartment" name="action" onclick="<%=request.getContextPath()%>/AdminController?departmentid=<%=department.getDepartmentId()%>"> --%>
	</form>
</body>
<br></br>
